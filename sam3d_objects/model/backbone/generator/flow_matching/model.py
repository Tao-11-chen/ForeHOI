...
class FlowMatching(Base):
    ...
    def loss(self, x1: torch.Tensor, *args_conditionals, **kwargs_conditionals):
        ...
        def _safe_loss(fn, weight, pred, targ):
            # Replace NaN/Inf in inputs before computing loss
            if torch.isnan(targ).any():
                return 0
            else:
                pred = torch.nan_to_num(pred, nan=0.0, posinf=1.0, neginf=-1.0)
                targ = torch.nan_to_num(targ, nan=0.0, posinf=1.0, neginf=-1.0)
                loss_val = weight * fn(pred, targ)
                # Clamp to avoid exploding loss; skip NaN steps
                loss_val = torch.nan_to_num(loss_val, nan=0.0, posinf=0.0, neginf=0.0)

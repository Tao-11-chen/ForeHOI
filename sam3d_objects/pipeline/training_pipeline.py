...
class TrainingPipeline(pl.LightningModule):
    def __init__(
        ...
        logger.info(f"self.device: {torch.device(device)}")
        ...
        with device:
            ...
            if shape_model_dtype is None:
                self.shape_model_dtype = self._get_dtype(dtype)
            else:
            ...
            ss_generator = self.init_ss_generator(
                ss_generator_config_path, ss_generator_ckpt_path, device
            )
            slat_generator = self.init_slat_generator(
                slat_generator_config_path, slat_generator_ckpt_path, device
            )
            ss_decoder = self.init_ss_decoder(
                ss_decoder_config_path, ss_decoder_ckpt_path, device
            )
            ss_encoder = self.init_ss_encoder(
                ss_encoder_config_path, ss_encoder_ckpt_path, device
            )
            slat_decoder_gs = self.init_slat_decoder_gs(
                slat_decoder_gs_config_path, slat_decoder_gs_ckpt_path, device
            )
            slat_decoder_gs_4 = self.init_slat_decoder_gs(
                slat_decoder_gs_4_config_path, slat_decoder_gs_4_ckpt_path, device
            )
            slat_decoder_mesh = self.init_slat_decoder_mesh(
                slat_decoder_mesh_config_path, slat_decoder_mesh_ckpt_path, device
            )
    ...
    def init_ss_generator(self, ss_generator_config_path, ss_generator_ckpt_path, device='cuda'):
    ...
    def init_slat_generator(self, slat_generator_config_path, slat_generator_ckpt_path, device='cuda'):
        config = OmegaConf.load(
    ...
    def init_ss_encoder(self, ss_encoder_config_path, ss_encoder_ckpt_path, device='cuda'):
    ...
    def init_ss_decoder(self, ss_decoder_config_path, ss_decoder_ckpt_path, device='cuda'):
        # override to avoid problem loading
        config = OmegaConf.load(
    ...
    def init_slat_decoder_gs(
        self, slat_decoder_gs_config_path, slat_decoder_gs_ckpt_path, device='cuda'
    ):
    ...
    def init_slat_decoder_mesh(
        self, slat_decoder_mesh_config_path, slat_decoder_mesh_ckpt_path, device='cuda'
    ):
        return self.instantiate_and_load_from_pretrained(
            OmegaConf.load(
    ...
    def init_ss_condition_embedder(
        self, ss_generator_config_path, ss_generator_ckpt_path, device='cuda'
    ):
        conf = OmegaConf.load(
            os.path.join(self.workspace_dir, ss_generator_config_path)
        )
    ...
    def init_slat_condition_embedder(
        self, slat_generator_config_path, slat_generator_ckpt_path, device='cuda'
    ):
    ...
    def training_step(self, batch, batch_idx):
        ...
        return self.models["ss_generator"].loss(batch)

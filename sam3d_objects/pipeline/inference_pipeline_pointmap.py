...
class InferencePipelinePointMap(InferencePipeline):
    ...
    def __init__(
        self, *args, depth_model=None, layout_post_optimization_method=None, clip_pointmap_beyond_scale=None, **kwargs
    ):
...
class InferencePartPipelinePointMap(InferencePipeline):
    ...
    def compute_scale_offset(self, part_pcd, global_pcd):
        """
        求解 s 和 o 使得 || s * pcd1 + o - pcd2 ||^2 最小
        ...
        # 计算质心
        c1 = part_pcd.mean(dim=0)  # (3,)
        c2 = global_pcd.mean(dim=0)  # (3,)
        
        # 去质心
        pcd1_centered = part_pcd - c1
        pcd2_centered = global_pcd - c2

        # 计算 scale: s = sum(pcd1_centered · pcd2_centered) / sum(pcd1_centered · pcd1_centered)
        s = (pcd1_centered * pcd2_centered).sum() / (pcd1_centered * pcd1_centered).sum()
    ...
    def _encode_global_ss(self, ss_input_dict):
        gt_global_ss_latent = self.models['ss_encoder'](ss_input_dict['global_ss'])['z'].reshape(ss_input_dict['global_ss'].shape[0], 8, 4096).permute(0, 2, 1)
        cond_global_ss = self.models['global_ss_condition_embedder'](gt_global_ss_latent)
        cond_global_ss = cond_global_ss + self.condition_embedders['ss_condition_embedder'].idx_emb[1:2, None]
        return cond_global_ss

    def sample_sparse_structure_part(
        ...
        cond_global_ss = self._encode_global_ss(ss_input_dict)
        if use_distillation:
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=self.shape_model_dtype):
                ...
                shape_latent = return_dict["shape"][:,:,:8]
                ss = ss_decoder(
                    shape_latent.permute(0, 2, 1)
                    .contiguous()
                    .view(shape_latent.shape[0], 8, 16, 16, 16)
                )
                xyz_latent = return_dict["shape"][:,:,8:16]
                xyz_coord = self.models['ss_decoder_xyz'](
                    xyz_latent.permute(0, 2, 1)
                    .contiguous()
                    .view(xyz_latent.shape[0], 8, 16, 16, 16)
                )
                coords = torch.argwhere(ss > 0)[:, [0, 2, 3, 4]].int()
                xyz = xyz_coord[0, 0:, coords[:,1], coords[:,2], coords[:,3]].t().contiguous()
                s, o = self.compute_scale_offset((coords[:, 1:].float() + 0.5) / 64 - 0.5, xyz)
                return_dict["scale_from_xyz"] = 1/s
                return_dict["translation_from_xyz"] = o
    ...
    def preprocess_image(
        ...
        rgba_image = torch.from_numpy(image)
        rgba_image = rgba_image.contiguous()
        rgb_image = rgba_image[:, :3]
        rgb_image_mask = get_mask(rgba_image, None, "ALPHA_CHANNEL")

        rgb_image, rgb_image_mask = list(rgb_image.unbind(dim=0)), list(rgb_image_mask.unbind(dim=0))
        if pointmap is not None and preprocessor.pointmap_transform != (None,):
            pointmap = list(pointmap.unbind(dim=0))
        else:
            pointmap = [None] * len(rgb_image)
        masks = []
        images = []
        rgb_images = []
        rgb_image_masks = []
    ...
    def run(
        ...
        image: Union[None, Image.Image, np.ndarray],
        mask: Union[None, Image.Image, np.ndarray] = None,
        global_ss: Union[None, np.ndarray] = None,
        seed: Optional[int] = None,
        stage1_only=False,
        with_mesh_postprocess=True,
        with_texture_baking=True,
        ...
        with self.device: 
            if normal is not None:
                normal = self.merge_image_and_mask(normal, mask)
                ss_input_dict = self.preprocess_image(
                    normal, self.ss_preprocessor, pointmap=pointmap
                )
            else:
                ss_input_dict = self.preprocess_image(
                    image, self.ss_preprocessor, pointmap=pointmap
            ...
            if seed is not None:
                torch.manual_seed(seed)
            ss_input_dict['global_ss'] = global_ss.to(self.device)
            ss_return_dict = self.sample_sparse_structure_part(
                ss_input_dict,
                inference_steps=stage1_inference_steps,
                use_distillation=use_stage1_distillation,
            )
...
class InferencePartPipelinePointMap_compress(InferencePartPipelinePointMap):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.models['global_ss_condition_type_emb'] = emb_module.to(self.device)
        self.condition_embedders["ss_condition_embedder"].module_list[2].input_size = 296
    ...
    def sample_sparse_structure_part(
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=self.shape_model_dtype):
                ...
                condition_args = (self.fuse_cond(condition_args[0]),)
                condition_args = (torch.cat([condition_args[0], cond_global_ss], dim=1),)
                return_dict = ss_generator(
                    latent_shape_dict,
                    image.device,

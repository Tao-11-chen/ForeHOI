...
class ObjectCentricSSI(SSIPointmapNormalizer):
    ...
    def _batch_compute_scale_and_shift(self, pointmap: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        pointmap_size = (pointmap.shape[2], pointmap.shape[3])
        ...
        bs = pointmap.shape[0]
        pointmap_flat = pointmap.permute(0,2,3,1).reshape(bs,-1,3)
        # Get valid points from the mask
        mask_num = mask_resized.reshape(bs,-1).sum(-1, keepdim=True)
        mask_weights = mask_resized.reshape(bs,-1) / (mask_num + 1e-5)
        mask_points = pointmap_flat * mask_weights.unsqueeze(-1)
        ...
        if self.use_scene_scale == True:
            # Normalize by the scene scale
            points_centered = pointmap_flat - shift.unsqueeze(1)
            max_dims = points_centered.abs().max(dim=-1)[0]
            scale = (max_dims * mask_weights).sum(1)
        elif self.use_scene_scale == False:
            # Normalize by the object scale
            shifted_mask_points = mask_points - shift.unsqueeze(-1)
            norm = shifted_mask_points.norm(dim=0)
            ...
            raise ValueError(f"Invalid use_scene_scale: {self.use_scene_scale}")
        scale = scale[:, None].repeat(1,3) # per-dim scaling
        scale = scale * self.scale_factor
        return scale, shift

    def batch_normalize(self, pointmap: torch.Tensor, mask: torch.Tensor,
        scale: Optional[torch.Tensor] = None, shift: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        # 1. resize mask to size of pointmap using nearest interpolation
        # 2. get mask points: pointmap[mask > 0.5]
        ...
        # 4. scale = # filter. If no points, then
        # logger.info(f"{pointmap.shape=} {mask.shape=}")
        assert pointmap.shape[1] == 3, "pointmap must be in (B, 3, H, W) format"

        _scale, _shift = self._batch_compute_scale_and_shift(pointmap, mask)
        if scale is not None and self.allow_scale_and_shift_override:
            _scale = scale
        if shift is not None and self.allow_scale_and_shift_override:
            _shift = shift
        ...
        # Apply normalization
        pointmap_normalized = _batch_apply_metric_to_ssi(pointmap, return_scale, return_shift) #  (B, 3, H, W)
        
        if self.clip_beyond_scale is not None and self.clip_beyond_scale > 0:
            new_norm = pointmap_normalized.norm(dim=1)
            pointmap_normalized = torch.where(
                new_norm > self.clip_beyond_scale,
                torch.full_like(pointmap_normalized, float('nan')),
                pointmap_normalized
...
def _batch_apply_metric_to_ssi(pointmap: torch.Tensor, scale: torch.Tensor, shift: torch.Tensor, apply_inverse: bool = False) -> torch.Tensor:
    """
    Normalize pointmap using Scale-Shift Invariant (SSI) normalization.
    ...
    if pointmap.shape[1] == 3:
        pointmap_hw3 = pointmap.permute(0,2,3,1)
        original_format = 'chw'
    else:
        pointmap_hw3 = pointmap
        original_format = 'hwc'
    ...
    bs, h, w, _ = pointmap_hw3.shape
    pointmap_flat = pointmap_hw3.reshape(bs, -1, 3)
    pointmap_normalized = transform_to_apply.transform_points(pointmap_flat).reshape(bs, h, w, 3)
    
    # Reshape back to original format
    if original_format == 'chw':
        pointmap_normalized = pointmap_normalized.permute(0, 3, 1, 2)
    else:
        pointmap_normalized = pointmap_normalized

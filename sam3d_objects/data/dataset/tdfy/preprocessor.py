...
class PreProcessor:
    ...
    def _normalize_pointmap(
        ...
        if self.normalize_pointmap == False:
            ...
            _, pointmap_scale, pointmap_shift = pointmap_normalizer.batch_normalize(pointmap, mask) if batch_mode else pointmap_normalizer.normalize(pointmap, mask)
            return pointmap, pointmap_scale, pointmap_shift
        
        if scale is not None or shift is not None:
            return pointmap_normalizer.batch_normalize(pointmap, mask, scale, shift) if batch_mode else pointmap_normalizer.normalize(pointmap, mask, scale, shift)
            
        return pointmap_normalizer.batch_normalize(pointmap, mask) if batch_mode else pointmap_normalizer.normalize(pointmap, mask)

    def _process_image_mask_pointmap_mess(
        self, rgb_image, rgb_image_mask, pointmap=None, normalize_pointmap=True, batch_mode=False
    ):
        """Extended version that handles pointmaps"""
 
        # Apply pointmap normalization if enabled
        if normalize_pointmap:
            pointmap_for_crop, pointmap_scale, pointmap_shift = self._normalize_pointmap(
                pointmap, rgb_image_mask, self.pointmap_normalizer, batch_mode=batch_mode
            )
        else:
            pointmap_for_crop = pointmap
            if pointmap is not None:
                pointmap_scale, pointmap_shift = torch.tensor([1.0, 1.0, 1.0]).to(pointmap.device), torch.tensor([0.0, 0.0, 0.0]).to(pointmap.device)
            else:
                pointmap_scale, pointmap_shift = None, None
        ...
        processed_rgb_image, processed_mask, processed_pointmap = (
            self._preprocess_image_mask_pointmap(rgb_image, rgb_image_mask, pointmap_for_crop, batch_mode=batch_mode)
        )
        ...
        rgb_pointmap, rgb_pointmap_scale, rgb_pointmap_shift = self._normalize_pointmap(
            pointmap, rgb_image_mask, self.rgb_pointmap_normalizer, pointmap_scale, pointmap_shift, batch_mode=False
        )
    ...
    def _preprocess_image_mask_pointmap(self, rgb_image, mask_image, pointmap=None, batch_mode=False):
        ...
        if (
            ...
            if batch_mode:
                rgb_image, mask_image, pointmap = list(rgb_image.unbind(dim=0)), list(mask_image.unbind(dim=0)), list(pointmap.unbind(dim=0))
                for i in range(len(rgb_image)):
                    for trans in self.img_mask_pointmap_joint_transform:
                        rgb_image[i], mask_image[i], pointmap[i] = trans(
                            rgb_image[i], mask_image[i], pointmap=pointmap[i]
                            )
                rgb_image = torch.stack(rgb_image, dim=0)
                mask_image = torch.stack(mask_image, dim=0)

...
from sam3d_objects.pipeline.training_pipeline_pointmap import *
...
def load_mask(path, ratio=-1):
    mask = load_image(path)
    if ratio != -1:
        mask_upsample = torch.tensor(mask).unsqueeze(0).unsqueeze(0).float()
        mask_upsample = torch.nn.functional.interpolate(
            mask_upsample, scale_factor=ratio, mode="nearest"
        )
        mask = mask_upsample[0, 0].numpy()
    mask = mask > 0
    if mask.ndim == 3:
        mask = mask[..., -1]
    return mask
...
def load_model(config_file: str, device: str = 'cpu', compile: bool = False):
    config = OmegaConf.load(config_file)
    config.rendering_engine = "pytorch3d"  # overwrite to disable nvdiffrast
    config.compile_model = compile
    config.workspace_dir = os.path.dirname(config_file)
    config.device = device
    config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingSSPipeline"
    check_hydra_safety(config, WHITELIST_FILTERS, BLACKLIST_FILTERS)
    pipeline: TrainingSSPipeline = instantiate(config)
    return pipeline

def load_model_part(config_file: str, device: str = 'cpu', compile: bool = False, depth: bool = False):
    config = OmegaConf.load(config_file)
    config.rendering_engine = "pytorch3d"  # overwrite to disable nvdiffrast
    config.compile_model = compile
    config.workspace_dir = os.path.dirname(config_file)
    config.device = device
    config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingMVSSPipeline"
    # depth-conditioning: 4-stream cond (ctx 4096) + resident MoGe for the
    # `moge` training mode. Default keeps ctx 3072 and deletes MoGe.
    OmegaConf.set_struct(config, False)
    config.use_depth = bool(depth)
    config.cond_ctx_channels = 4096 if depth else 3072
    check_hydra_safety(config, WHITELIST_FILTERS, BLACKLIST_FILTERS)
    pipeline: TrainingMVSSPipeline = instantiate(config)
    return pipeline